...
- fact: An independent critic replaced a `_readings_of_*` helper with two literals equal to the two values its own entry carries and found every instrument green, the full suite included. Measured here by the class's own helper, which builds that tree -- the tree minus this class's block, since the block carries the helper that runs the control -- and runs the control in it: `--check` and `--selftest` both exit 0, `halves not read against an entry` counts nothing. The reason is structural: the witness check is `helper() == entry`, so a helper that repeats the entry satisfies it by construction, and no instrument that only compares the two can separate computing from copying. What the control guards is a witness that DISAGREES with its entry -- a half replaced with a DIFFERENT value is refused, and the control's own self-test shows it. Read the promise at that strength: a half replaced by the entry's own values is unwitnessed and stays silent in every command in the tree. A measurement that runs the control that runs it is the same shape one level up, and it terminates only because the copy leaves the measuring block out.
- probe: `a_witness_that_repeats_the_verdict_it_is_compared_against()` -> expected `{'replacements_tried': 1, 'instruments_run': 2, 'instruments_that_refused_the_replacement': 0, 'halves_left_unwitnessed': 1, 'the_unwitnessed_half_is_byte_for_byte_the_entry': True, 'helpers_the_copy_counts': 12}`, observed `{'replacements_tried': 1, 'instruments_run': 2, 'instruments_that_refused_the_replacement': 0, 'halves_left_unwitnessed': 0, 'helpers_the_copy_counts': 12}`
- instances: 1
