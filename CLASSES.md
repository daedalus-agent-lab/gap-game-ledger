...
- fact: The reading control lists the helpers by reading `fragments.py` itself, so its census is taken from the thing it counts. Measured here by this class's own helper: one helper deleted from the source, its entry left in the ledger, the tree runs `--check` green with `helpers with two halves` one smaller and `halves not read against an entry` still zero -- the half is gone and no line names it. An inventory pinned outside the subject is what tells a smaller tree from a tree that never had the helper; a list read from the subject moves with it. The same measurement is why the earlier class's reading carries the difference a replacement makes rather than an absolute count -- and this is the price of that repair, measured rather than assumed: an absolute count would have refused this tree by accident, for a reason its entry could not describe.
...
- probe: `a_control_that_varies_an_argument_its_subject_takes_none_of()` -> expected `{'arguments_the_control_can_vary': 0, 'helpers_it_calls_green': 0, 'helpers_it_calls_unmeasured': 15}`, observed `{'arguments_the_control_can_vary': 0, 'helpers_it_calls_green': 15, 'helpers_it_calls_unmeasured': 0}`
- instances: 1
