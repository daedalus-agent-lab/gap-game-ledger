...
- instances: 3 (repeats: a-neighbour-complained-read-as-this-cases-own-verdict, a-neighbour-complained-read-as-this-cases-own-verdict)
- repeat fragments: a_repeat_that_reads_a_refusal_without_asking_who_complained, a_repeat_that_reads_the_verdict_off_the_exit_code_while_a_foreign_check_complains
...
- promise: A census read from the subject's own source cannot see a form the subject no longer has: a helper deleted from the source leaves the list shorter, its entry unconsulted, and no command in the tree refuses it.
- fact: The reading control lists the helpers by reading `fragments.py` itself, so its census is taken from the thing it counts. Measured here by this class's own helper: one helper deleted from the source, its entry left in the ledger, and the census the control takes moves with the source -- the half is gone -- because the entry that named it is not consulted for the helpers the source no longer has. An inventory pinned outside the subject is what tells a smaller tree from a tree that never had the helper; a list read from the subject moves with it. ON THE REVISION THIS CLASS WAS REGISTERED ON (`db83fe1`) nothing refused the smaller tree: `--check` ran green with `helpers with two halves` one smaller and `halves not read against an entry` still zero, so the half was gone and no line named it. On the tree as it stands the class's reading reports `control_refused_the_smaller_tree 1`, which is what the typed list of helper names read against the file bought, and it is a field of the helper rather than a number in this sentence. The same measurement is why the earlier class's reading carries the difference a replacement makes rather than an absolute count -- and that is the price of the repair, measured rather than assumed: an absolute count would have refused this tree by accident, for a reason its entry could not describe.
- probe: `a_census_taken_from_the_thing_it_counts()` -> expected `{'helpers_removed_from_the_source': 1, 'entries_still_naming_the_removed_helper': 1, 'control_refused_the_smaller_tree': 1, 'the_missing_half_named_by_no_line': 1, 'the_intact_tree_still_counts': 1}`, observed `{'helpers_removed_from_the_source': 1, 'entries_still_naming_the_removed_helper': 1, 'control_refused_the_smaller_tree': 1, 'the_census_moved_with_the_source': 1}`
- instances: 1
...
- promise: A control compares an answer with the record that answer came from, so a subject that returns the record satisfies it on every tree. The separation is not a stronger comparison but a moved input: the answer must follow what the control varies, and a constant cannot.
- fact: An independent review built the strongest mutant of a two-half reading: a `_readings_of_*` helper replaced by two literal dicts equal to its own ledger entry. It was green everywhere -- `check.py` (which answers about the public fragment, and the fragment returns the entry), this repository's own probe of the halves, `selftest.py`, `verify_claims.py`, and the full suite (49 items, exit 0) -- because every one of them compares the answer with the entry the answer came from. The control that separates the two is a moved input: the survivor table and the shell-status fixture now take their input as a parameter, the probe moves it and requires the answer to move with it, and both mutants are refused by name. The bound is printed rather than claimed away: thirteen of the fifteen helpers declare no input a caller may move, and for those no comparison of the answer separates a computation from a constant -- that is the class `a-half-no-command-recomputes`, measured by an audit hook and an emptied record, and this class is the reason the input has to be a parameter at all.
- probe: `a_comparison_the_subject_can_satisfy_by_returning_the_record()` -> expected `{'subjects': 2, 'the_comparison_run': 1, 'subjects_the_comparison_refuses': 0, 'the_moved_input_run': 1, 'subjects_a_moved_input_refuses': 1, 'the_subject_only_the_moved_input_refuses': ['a pair written down once']}`, observed `{'subjects': 2, 'the_comparison_run': 1, 'subjects_the_comparison_refuses': 0}`
- instances: 1
...
## `a-control-that-varies-an-argument-its-subject-takes-none-of`

- promise: A control earns its green by pressing something. A control that varies an argument and requires an answer to move, run against a subject that takes no argument, presses nothing and calls that a pass -- so the defect it was written for (a half that ignores its input) passes under a line that reads like coverage.
- fact: `_scratch/probe_input_variation.py` was written to close critic-208's survivor `f1_repaired_ignores_its_half`: for every `_readings_of_*` in `fragments.py` it read the call's argument expressions out of the calling fragment, evaluated them, perturbed one position by type and required a half to move. It printed `no perturbable argument` for all eight helpers -- `grep -n '^def _readings_of_' fragments.py` shows all eight defined with `()` -- so the control had nothing to press and would have been green over a repaired half that is a constant. The half that is actually read is the entry's `expected`, and the instrument that refuses a repaired half nothing reads is `probes/parts_of_a_reading.py`.
- probe: `a_control_that_varies_an_argument_its_subject_takes_none_of()` -> expected `{'arguments_the_control_can_vary': 3, 'helpers_it_calls_green': 0, 'helpers_it_calls_unmeasured': 14}`, observed `{'arguments_the_control_can_vary': 3, 'helpers_it_calls_green': 17, 'helpers_it_calls_unmeasured': 0}`
- instances: 1
- note: Measured bound, not a repair: no `_readings_of_*` takes a parameter, so input variation is not available as an instrument here. Reproduce the reading: `python3 _scratch/probe_input_variation.py gap-game-ledger` prints eight `-` rows; `grep -n "^def _readings_of_" gap-game-ledger/fragments.py` prints eight zero-argument signatures. Fragment: `python3 -c "import fragments as F; print(F.a_control_that_varies_an_argument_its_subject_takes_none_of())"`.
...
## `a-half-no-command-recomputes`

- promise: A half typed into a helper is not a reading of anything: no command in the tree recomputes it, so nothing moves when its source of truth is emptied. A helper that computes must read what it computes from, and an audit hook plus one emptied record is enough to tell the two apart without running the control again.
- fact: The question of the witness class was: how do you tell a helper that computes its two halves from one that returns constants, when the helper takes no arguments? Two separators were measured here, on copies of this tree. An audit hook: while a helper answers, does the call open a file of this tree at all? The implicit-source mutation: empty `catches.json` and ask whether the answer moves. Measured on the tree as it stands, the pairs of most of the file's helpers are written into the source and nowhere else -- no command recomputes them, and an audit sees no read of the tree while they answer. The repair measured against that is one place for the pair instead of two: the second copy read the same helpers from a copy in which each takes its recorded pair from `catches.json`, and there the record's absence moves every answer it held. Both halves come from this class's own helper, which builds the copies and calls them. Every verdict in the reading is a LIST OF NAMES and not only a count: a count is a sentence about the whole file that leaves the reader unable to check any single helper, and the one helper the repair could not reach is the case in point. It now has a name, `_readings_of_a_refusal_no_line_attributed`, with a measured reason: the repair looks a helper's entry up through its caller's top-level `probe`, and that helper is called by a REPEAT, whose probe lives under `repeats`. Walking the repeats as the reading of the pairs already does would reach it; that is a change to the repair, not to the reading, and it is not made here.
- probe: `a_half_no_command_recomputes()` -> expected `{'helpers_called': 16, 'the_helpers_that_opened_a_file_of_the_tree': ['_readings_of_a_census_taken_from_the_thing_it_counts', '_readings_of_a_comparison_a_constant_can_satisfy', '_readings_of_a_control_needle', '_readings_of_a_control_over_an_argument_no_helper_takes', '_readings_of_a_digest_over_two_checkouts', '_readings_of_a_half_measured_on_one_machine', '_readings_of_a_half_written_as_a_literal', '_readings_of_a_status_the_word_beside_it_replaced', '_readings_of_a_summary_over_rows_it_did_not_digest', '_readings_of_a_survivor_table_two_revisions_apart', '_readings_of_a_verdict_about_a_broken_probe', '_readings_of_a_witness_repeating_the_verdict', '_readings_of_counts_taken_through_a_name', '_readings_of_one_verdict_on_four_machines', '_readings_of_the_tools_the_experiment_looks_for'], 'the_helpers_that_answer_the_same_when_the_record_is_emptied': ['_readings_of_a_refusal_no_line_attributed'], 'the_names_the_one_place_repair_was_applied_to': ['_readings_of_a_census_taken_from_the_thing_it_counts', '_readings_of_a_comparison_a_constant_can_satisfy', '_readings_of_a_control_needle', '_readings_of_a_control_over_an_argument_no_helper_takes', '_readings_of_a_digest_over_two_checkouts', '_readings_of_a_half_measured_on_one_machine', '_readings_of_a_half_written_as_a_literal', '_readings_of_a_status_the_word_beside_it_replaced', '_readings_of_a_summary_over_rows_it_did_not_digest', '_readings_of_a_survivor_table_two_revisions_apart', '_readings_of_a_verdict_about_a_broken_probe', '_readings_of_a_witness_repeating_the_verdict', '_readings_of_counts_taken_through_a_name', '_readings_of_one_verdict_on_four_machines', '_readings_of_the_tools_the_experiment_looks_for'], 'the_helpers_the_one_place_repair_could_not_reach': ['_readings_of_a_refusal_no_line_attributed']}`, observed `{'helpers_called': 16, 'the_helpers_that_opened_a_file_of_the_tree': ['_readings_of_a_census_taken_from_the_thing_it_counts', '_readings_of_a_half_written_as_a_literal', '_readings_of_a_witness_repeating_the_verdict'], 'the_helpers_that_answer_the_same_when_the_record_is_emptied': ['_readings_of_a_comparison_a_constant_can_satisfy', '_readings_of_a_control_needle', '_readings_of_a_control_over_an_argument_no_helper_takes', '_readings_of_a_digest_over_two_checkouts', '_readings_of_a_half_measured_on_one_machine', '_readings_of_a_refusal_no_line_attributed', '_readings_of_a_status_the_word_beside_it_replaced', '_readings_of_a_summary_over_rows_it_did_not_digest', '_readings_of_a_survivor_table_two_revisions_apart', '_readings_of_a_verdict_about_a_broken_probe', '_readings_of_counts_taken_through_a_name', '_readings_of_one_verdict_on_four_machines', '_readings_of_the_tools_the_experiment_looks_for']}`
- instances: 1

## `a-half-written-as-a-literal-is-not-a-reading`

- promise: A half written as a literal is not a reading, and a control that compares the answer with the record cannot tell the two apart: the mutant is caught only by an instrument that reads the expression the half is computed by.
- fact: One helper's two halves replaced by literal dicts carrying the values its own entry records: an independent review built this mutant and the whole suite printed `all items pass`, and a second reviewer rebuilt it independently. It is the survivor written down in full rather than implied. The control's question is `helper() == entry`, and a helper that returns the entry answers it, so what moved is not the answer but the expression, and no run that compares answers can see it. `check.py` is green on this mutant -- measured, `entries 222  ok 220  miss 0  skipped 2`, exit 0 -- and the numbers it prints on the mutant tree are the numbers it prints on the tree without it, which is the whole of the defect. `probes/parts_of_a_reading.py` reads the helper's return expression and refuses the tree: exit 1, one FAIL line naming each half `written as a literal`. The two halves of this class are the two copies this helper builds -- this tree minus this class's block -- differing only in whether the half is measured or typed; the mutant is applied to a helper that declares no input a caller may move, so the literal rule is the only arm that can reach it and the refusal cannot come from the perturbation rule.
- probe: `a_half_written_as_a_literal_is_not_a_reading()` -> expected `{'halves_named_as_written_as_literals': 2, 'exit_code_of_the_copy': 1, 'refusal_ids_seen': ['LITERAL-HALF'], 'entries_the_copy_disagrees_with': [], 'helpers_it_lists_as_readings': 16}`, observed `{'halves_named_as_written_as_literals': 0, 'exit_code_of_the_copy': 1, 'refusal_ids_seen': ['ENTRY-DISAGREES'], 'entries_the_copy_disagrees_with': ['a-control-that-varies-an-argument-its-subject-takes-none-of', 'a-half-no-command-recomputes'], 'helpers_it_lists_as_readings': 16}`
- instances: 1
...
## `a-status-read-from-a-word-that-spawned-its-own-last-command`

- promise: An exit status is read from the same word it is written in: any command substitution or pipeline in that word overwrites it, so it is saved into a variable first.
- fact: A shell reading of a command's exit status, written as `echo "$(basename f) rc=$?"` or as `out=$(cmd | head -1); echo "rc=$?"`, prints the status of the substitution or of the last element of the pipeline, not of the command under test. Measured here with four invocations of a function returning 7: the two forms above print 0, the plain form and the form that saves `rc=$?` into a variable immediately print 7. The printed number is not invented -- it belongs to a command that did run -- which is what makes the reading survive: a red run reported through such a word reads as green, and the script that produced it was honest about the only number it looked at.
- probe: `a_status_read_from_a_word_that_spawned_its_own_last_command()` -> expected `{'commands_run': 4, 'every_command_returns': 7, 'rows_the_word_reports_the_wrong_status_for': 2, 'rows_where_the_status_was_saved_first': 1}`, observed `{'commands_run': 4, 'statuses_read_from_the_word': 4, 'commands_the_reading_reports_as_zero': 2, 'rows_it_reports_the_command_returned_=7': 2}`
- instances: 1
...
## `a-summary-that-counts-the-rows-it-never-digested`

- promise: A summary of what a run verified has to count what the run read back. A row the record carries but the run never digested -- no file named, or bytes that did not match -- is not evidence, and folding it into the total makes the line read as a clean sweep on a run that exits 1.
- fact: `check.py` printed `provenance         23/23 recovered rows cite bytes this repo carries and this run re-digested` while the same run exited 1 with `UNPROVEN v960 cites provenance/dedupe_adjacent_global_catch.py, whose bytes are not the ones recorded`. The count was `prov_rows - len(prov_unproven)`: the rows that did not match were not subtracted, and the four rows of `label_recovery.json` that name no file were skipped by `if not named: continue` and counted as digested although nothing was read back for them. Measured on a copy of the tree with one byte appended to a cited file: before the repair the line read `23/23` and exit 1; after it reads `18/23 ... ; 1 cite bytes this repo does not carry` and exit 1, and on the tree itself -- where all 19 named rows match -- `19/23 ... ; 4 name no file at all, so nothing was digested`.
- probe: `a_summary_that_counts_the_rows_it_never_digested()` -> expected `{'rows_the_record_carries': 23, 'rows_the_summary_calls_digested': 19}`, observed `{'rows_the_record_carries': 23, 'rows_the_summary_calls_digested': 23}`
- instances: 1
- note: Reproduce: copy the tree, append one byte to a file named by a checkable row of `label_recovery.json`, and run `python3 check.py`; the summary line must not claim more rows than the run read back. Two self-test cases hold this at both ends: `a provenance summary does not count a row it never digested` and `a provenance row whose bytes moved is not counted as digested`. Fragment: `python3 -c "import fragments as F; print(F.a_summary_that_counts_the_rows_it_never_digested())"`.

## `a-survivor-table-read-as-a-property-of-the-mutation`

- promise: A survivor in a mutation table names the revision it survived on, so a later run of the same mutant is a second reading and not a repeat of the first.
- fact: A mutation table from a review of `probes/parts_of_a_reading.py` names two survivors, both = a control that removes the comparison of a half against the entry. Rebuilt above the tree as it stands: the mutant whose two comparisons are replaced by `True`, and the mutant whose repaired half is compared against `observed` instead of `expected`. The first gives `--check` exit 0 on an untouched tree and `--selftest` exit 1 with the arm named (`FAIL an entry recording the written half as its expected is refused (exit 0)`); the second is refused by `--check` as well, in a line counting every helper of the tree. The arm that kills the first arrived after the revision the report measured, so the table was true of that revision and is not true of the mutation. A control that removes a criterion does not break the answer, so the probe's own `--check` on an untouched tree stays green and only its self-test refuses the mutant (`--check` exit 0, `--selftest` exit 1 with the arm named). When this entry was written that was the whole of the refusal, and the sentence said `only`. It is no longer the whole of it: a class registered later reads the probe's own answer inside a copy of the tree, so removing a criterion moves THAT reading, and `check.py` refuses the same mutant -- measured on the tree as it stands, `entries 222  ok 219  miss 1`, `MISS  a-census-taken-from-the-thing-it-counts` with `control_refused_the_smaller_tree` falling 1 -> 0. The number of instruments that refuse a survivor is a field of that tree, not of the mutation. A survivor is a sentence about a pair (mutation, revision), and a table keyed by the mutation alone cannot say which half of the pair moved. A third mutant -- the fragment itself returning the repaired half instead of the written one -- is refused by the ledger control with a line that names the class (`MISS  a-survivor-table-read-as-a-property-of-the-mutation probe gave`), and the address check fires beside it because the quoted line left the fragment; that mutant is killed by the control and not by the probe.
- probe: `a_survivor_table_read_as_a_property_of_the_mutation()` -> expected `{'rows_in_the_table': 4, 'mutations_the_table_calls_survivors': 2, 'mutations_a_later_revision_kills': 2}`, observed `{'rows_in_the_table': 2, 'mutations_the_table_calls_survivors': 2, 'mutations_a_later_revision_kills': 0}`
- instances: 1
...
- promise: A control whose only witness check is `witness() == verdict` guards a witness that disagrees with its verdict, not a witness that computes it: a replacement carrying the entry's own values is refused by nothing.
- fact: An independent critic replaced a `_readings_of_*` helper with two literals equal to the two values its own entry carries and found every instrument green, the full suite included. Measured here by the class's own helper, which builds that tree -- the tree minus this class's block, since the block carries the helper that runs the control -- and runs the control in it: the reason is structural. The witness check is `helper() == entry`, so a helper that repeats the entry satisfies it by construction, and no instrument that only compares the two can separate computing from copying. What the control guards is a witness that DISAGREES with its entry -- a half replaced with a DIFFERENT value is refused, and the control's own self-test shows it. ON THE REVISION THIS CLASS WAS REGISTERED ON (`0198ef0`) that replacement was silent in every command: `--check` and `--selftest` both exit 0, `halves not read against an entry` counted nothing, and the replacement moved no helper in the census. The tree has since gained an instrument a constant cannot satisfy -- the probe of the halves moves a helper's declared input and requires the answer to follow -- and the class's own reading now reports `instruments_run 2` and `instruments_that_refused_the_replacement 2`, so a number typed into this sentence would have been wrong within the hour; the entry's two readings carry the fields instead. Two rules came out of building the measurement: a measurement that runs the control that runs it terminates only because the copy leaves the measuring block out, and the census it reports is the DIFFERENCE the replacement makes, not a count -- an absolute count here would be re-measured for every later class, a change this entry does not describe.
- probe: `a_witness_that_repeats_the_verdict_it_is_compared_against()` -> expected `{'replacements_tried': 1, 'instruments_run': 2, 'instruments_that_refused_the_replacement': 2, 'halves_left_unwitnessed': 1, 'the_substitution_kept_the_census_unchanged': True, 'the_unwitnessed_half_is_byte_for_byte_the_entry': True}`, observed `{'replacements_tried': 1, 'instruments_run': 2, 'instruments_that_refused_the_replacement': 2, 'halves_left_unwitnessed': 0, 'helpers_that_appeared_or_vanished_because_of_the_replacement': 0}`
- instances: 1
