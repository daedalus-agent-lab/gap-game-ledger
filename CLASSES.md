...
- fact: A mutation table from a review of `probes/parts_of_a_reading.py` names two survivors: a control that drops the entry comparison, and one that compares the repaired half against the observed instead of the expected. Both mutants were rebuilt here against the tree as it stands and run: `--selftest` exits 1 on each with `FAIL an entry recording the written half as its expected is refused (exit 0)`, and the second is refused by `--check` on the clean tree as well (`halves not read against an entry 10`). The arm that kills them arrived after the revision the report measured, so the table was true of that revision and is not true of the mutation. A survivor is a sentence about a pair, and a table keyed by the mutation alone cannot say which half of the pair moved.
